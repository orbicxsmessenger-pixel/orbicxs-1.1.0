...
<div class="app">
  ...
  <div id="authStep">
    ...
    <div class="content">
      ...
      <h3 > TEXNIK ISHLAR OLIB BORILMOQDA</h3>
    </div>
  </div>
  ...
  <div id="profileStep" class="hidden">
    <div class="header">
      <span class="back" onclick="signOut()">Back</span>
      Profilni to'ldiring
    </div>
    <div class="content">
  ...
  <div id="usersStep" class="hidden">
    ...
    <div class="search">
      <input type="text" placeholder="Ism bo'yicha qidirish..." oninput="searchUsers(this.value.trim())">
    </div>
    <div class="user-list" id="userList">
      <p style="text-align:center;padding:50px;color:#777;">Ism kiriting...</p>
    </div>
  </div>
  ...
  <div id="chatStep" class="hidden chat-view">
    ...
    <div class="input-bar">
      ...
      <button onclick="document.getElementById('imgInput').click()" title="Rasm yuborish">Attach</button>
...
<script>
  ...
  let me = null;           // hozirgi foydalanuvchi (user object)
  let myUsername = null;   // foydalanuvchi ismi
  let currentChat = null;  // hozirgi chatdagi odam ismi
  ...
  async function sendMagicLink() {
    ...
    status('Magic havola emailingizga yuborildi! Tekshiring', '#4caf50');
  }
  ...
  async function saveProfile() {
    ...
    if (!username || username.length < 2) {
      document.getElementById('profileError').textContent = 'Ism kamida 2 harfdan iborat bo‘lsin!';
      return;
    }
    ...
    if (error) {
      document.getElementById('profileError').textContent = 'Xato: ' + error.message;
      return;
    }

    // Muvaffaqiyatli saqlansa – avto o‘tish
    myUsername = username;
    hideAll();
  ...
  async function searchUsers(q) {
    ...
    if (q.length < 2) {
      list.innerHTML = '<p style="text-align:center;padding:50px;color:#777;">Ism kiriting...</p>';
      return;
    }
    ...
    if (!data || data.length === 0) {
      list.innerHTML = '<p style="text-align:center;padding:50px;color:#777;">Hech kim topilmadi</p>';
      return;
    }

    data.forEach(u => {
      const diff = (Date.now() - new Date(u.last_seen)) / 60000;
      const status = diff < 3 ? '<span class="online">online</span>' : 'oxirgi faollik: ' + new Date(u.last_seen).toLocaleTimeString('uz');
  ...
  async function sendImage(e) {
    ...
    const { data } = supabase.storage.from('chat_images').getPublicUrl(path);

    await supabase.from('messages').insert({
      chat_id: chatId,
      sender: myUsername,
      text: `<img src="${data.publicUrl}" style="max-width:100%;border-radius:12px;margin:5px 0;">`
    });
  }

  // Xabarlarni yuklash va real-time tinglash
  ...
  // Eng muhim qism: Auth holatini doimiy kuzatish
  ...
  // Sahifa ochilganda ham bir marta tekshirish (ixtiyoriy)
  window.addEventListener('load', () => {
    supabase.auth.getSession(); // onAuthStateChange ni trigger qiladi
  });
</script>
</body>
</html>
